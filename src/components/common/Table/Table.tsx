...
const Table: React.FC<TableProps> = ({ columns = [], rowKey, data = [], loading, deleteData }) => {
        ...
        <thead>
          <tr>
            {columnsToDisplay.map((column, index) => (
              <th
                key={index}
                className="p-3 text-left bg-gray-100"
              >
                {column.label}
              </th>
            ))}
                ...
                {columnsToDisplay.map((column, index) => (
                  ...
                  >
                    {column.field === 'actions' ? (
                      <div>
                        <button className="px-4 py-2 bg-green-500 text-white rounded mr-2 my-2">Edit</button>
